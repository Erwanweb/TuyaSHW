...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if 4 not in Devices:
            Domoticz.Device(Name="Volume", Unit=4, Type=243, Subtype=6, Used=1).Create()
        ...
        # set cold water temp
        self.TempMini = Parameters["Mode3"]
        self.TempMini = float(self.TempMini)
        self.TempMini = round(self.TempMini)
...
# ---------------------------------------------
    def onHeartbeat(self):
        ...
        if self.CheckDeviceRequest + timedelta(seconds=20) <= now:
            self.checkdevice()
