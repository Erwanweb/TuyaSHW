...
"""
<plugin key="RL-TuyaSHW" name="Ronelabs - TUYA SHW device control plugin" author="Ronelabs" version="0.0.2" externallink="https://ronelabs.com">
      <description>
...
# ---------------------------------------------
    def checkdevice(self):
        ...
        if self.SpecFolder == "" :
            ...
            cmd = 'sudo python3 /home/domoticz/plugins/TuyaSHW/checkdevice.py'
            output = sp.getoutput(cmd)
        else :
            PluginFolder = str(self.SpecFolder)
            Domoticz.Debug(f"Using special Folder json Forecast file : {PluginFolder}")
            cmd = f'sudo python3 {PluginFolder}checkdevice.py'
            output = sp.getoutput(cmd)

        if output :
            Power = str(output['dps']['1'])
            if power == "True":
                self.powerOn = 1
                Devices[1].Update(nValue=self.powerOn, sValue=str(Level))
            else :
            ...
            self.setpoint = str(output['dps']['2'])
            Devices[2].Update(nValue=0, sValue="{}".format(str(self.setpoint)))
            self.watertemp = str(output['dps']['3'])
...
# ---------------------------------------------
    def tuyaorder(self):
        ...
        if self.SpecFolder == "" :
            ...
            PluginFolder = str(self.SpecFolder)
            Domoticz.Debug(f"Using special Folder for Setting TUYA Device : {PluginFolder}")
            with open(f'{PluginFolder}tuyadevice.py', 'w') as f:
                f.write(f"# Orders variable for tuya device\n")
                f.write(f"#\n")
                f.write(f"Power = '{Power}'\n")
                f.write(f"Setpoint = '{Setpoint}'\n")
