...
# ---------------------------------------------
    def checkdevice(self):
        ...
        #Domoticz.Debug(f'{DEVICE_ID}, {IP_ADDRESS}, {LOCAL_KEY}, version={VERSION}')
        #d = tinytuya.Device('bfcc28923f9cbde8281cd4', '192.168.0.118', f"'{LOCAL_KEY}'", version=3.4)
        #d = tinytuya.Device(f'{DEVICE_ID}, {IP_ADDRESS}, {LOCAL_KEY}, version={VERSION}')
        d = tinytuya.Device("'"+str(self.DEVICE_ID)+"'", "'"+str(self.IP_ADDRESS)+"'", "'"+str(self.LOCAL_KEY)+"'", version=float(self.VERSION))
        Domoticz.Debug(f'checking : tinytuya.Device({d})')
        #d = tinytuya.Device(f'{DEVICE_ID}', f'{IP_ADDRESS}', f'{LOCAL_KEY}', version=f'{VERSION}')
        #d = tinytuya.Device("{}, {}, {}, version={}").format(DEVICE_ID, IP_ADDRESS, LOCAL_KEY, VERSION))
