...
# ---------------------------------------------
    def checkdevice(self):
        ...
        power = data['dps']['1'])
        self.setpoint = data['dps']['2'])
        self.watertemp = data['dps']['3'])
        ...
        """if self.SpecFolder == "" :
            ...
            commande = 'sudo python3 /home/domoticz/plugins/TuyaSHW/checkdevice.py'
            #output = sp.getoutput(cmd)
        else :
            PluginFolder = str(self.SpecFolder)
            Domoticz.Debug(f"Using special Folder json Forecast file : {PluginFolder}")
            commande = f'sudo python3 {PluginFolder}checkdevice.py'
            #output = sp.getoutput(cmd)

        # Exécution de la commande et capture de la sortie JSON
        resultat = subprocess.run(commande, shell=True, capture_output=True, text=True)

        # Conversion de la sortie de la commande en JSON
        try:
            json_resultat = json.loads(resultat.stdout)
            Domoticz.Debug(f"Using special Folder json Forecast file : {json_resultat}")

        except json.JSONDecodeError:
            Domoticz.Error("La sortie de la commande n'est pas un JSON valide.")

        # Mise à jour des widgets
        power = str(json_resultat['dps']['1'])
        if power == "True":
            self.powerOn = 1
            Devices[1].Update(nValue=self.powerOn, sValue=str(Level))
        else :
            self.powerOn = 0
            Devices[1].Update(nValue=self.powerOn, sValue=str"0")
        self.setpoint = str(json_resultat['dps']['2'])
        Devices[2].Update(nValue=0, sValue="{}".format(str(self.setpoint)))
        self.watertemp = str(json_resultat['dps']['3'])
